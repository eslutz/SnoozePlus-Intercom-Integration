...
describe('Crypto Utility Functions', () => {
  ...
  describe('decrypt', () => {
    ...
    test('should throw error for invalid format', () => {
      expect(() => decryptText('invalid', algorithm, key)).toThrow(
        'Invalid encrypted text format. Expected format: ivHex:encrypted'
      );
      expect(() =>
        decryptText('invalid:format:too:many:parts', algorithm, key)
      ).toThrow(
        'Invalid encrypted text format. Expected format: ivHex:encrypted'
      );
    });
    ...
    test('should throw error for missing parts', () => {
      expect(() => decryptText(':', algorithm, key)).toThrow(
        'Invalid encrypted text format. Missing IV or encrypted data'
      );
      expect(() => decryptText('onlyonepart', algorithm, key)).toThrow(
        'Invalid encrypted text format. Expected format: ivHex:encrypted'
      );
    });
  });
